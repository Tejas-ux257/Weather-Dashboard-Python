...
class WeatherDashboard:
    def __init__(self, root):
        ...
        # ==== API CONFIG ====
        # TODO: put your real OpenWeatherMap API key below as a string
        self.api_key = "a8c5515023f8f0438ffb971686b396da"
    ...
    def setup_ui(self):
        ...
        viz_frame = tk.LabelFrame(
            self.root,
            text="Data Visualizations (5-day / 3-hour forecast)",
            font=("Arial", 14, "bold"),
            bg="white",
            fg="#333",
        )
    ...
    def fetch_weather(self):
        ...
        if not city or city.lower() == "city name":
        ...
        if not self.api_key or self.api_key == "YOUR_API_KEY_HERE":
            messagebox.showerror(
                "API Key Missing",
                "Please set your OpenWeatherMap API key in the code before running.",
            )
            return
        ...
        try:
            ...
            if current_response.status_code == 404:
                ...
                self.status_bar.config(text="Error: City not found")
                return
    ...
    def create_visualizations(self):
        """Create line, bar and pie charts using the forecast & current data."""
        if not self.forecast_data or "list" not in self.forecast_data:
            return

        self.fig.clear()

        # Use the next 8 *3-hour* intervals (~24 hours)
        ...
        ax1.plot(times, temps, marker="o", linewidth=2, markersize=8,
                 color="#FF5722", label="Temperature")
        ax1.plot(times, feels_like, marker="s", linewidth=2, markersize=6,
                 color="#2196F3", linestyle="--", label="Feels Like")
        ...
        # ---- Pie chart: Temperature breakdown ----
    ...
    def save_data(self):
        """Save current+forecast weather data to a JSON file."""
        if not self.weather_data or not self.forecast_data:
            messagebox.showwarning(
                "No Data",
                "No weather data to save.\n\nFetch a city first, then try again.",
            )
            return
        ...
        try:
            with open(filename, "w", encoding="utf-8") as f:
                json.dump(
                    {"current": self.weather_data, "forecast": self.forecast_data},
                    f,
                    indent=4,
                )
